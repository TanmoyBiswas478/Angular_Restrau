...
//     // Clean payload matching Laravel Validator strictly
//     const payload: any = {
...
//   // 🛡️ STRICT TYPE SAFE START EDIT METHOD
...
//     // 🟢 Fix 1: Safe fallback for status
//     this.newStatus = emp.status || 'Active';

//     // 🟢 Fix 2: Safe string fallback for phone before split
...
//       // 🟢 Fix 3: Safe assignment to newPhone
...
//   updateStatus(emp: Employee, newStatus: string) {
//     if (!emp.id) return;
//     const oldStatus = emp.status;
//     emp.status = newStatus as EmployeeStatus;
//     this.cdr.detectChanges();

//     this.employeeService.updateEmployee(emp.id, { status: newStatus as EmployeeStatus }).subscribe({
//       error: (err: any) => {
...
export class EmployeesComponent implements OnInit {
  ...
  // 🟢 CLEAN SINGLE DECLARATION (No Duplicates)
  employees: Employee[] = [];
  ...
  // 📊 Fetch Employees with Total Count Calculation
  fetchEmployees() {
    this.isLoading = true;
    this.employeeService.getEmployees().subscribe({
      next: (data: Employee[]) => {
        this.employees = data || [];
        this.totalEmployees = this.employees.length; // 👈 Dynamic row count
        this.isLoading = false;
        this.cdr.detectChanges();
      },
      error: (err: any) => {
